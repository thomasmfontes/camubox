    ...
    try {
      ...
      if (correlationID && process.env.SUPABASE_URL) {
        ...
        if (correlationID.startsWith('EXC_')) {
          ...
          const { data: lockerInfo } = await supabase.from('v_armario').select('cd_armario, nr_armario').eq('id_armario', newLockerId).maybeSingle();
          ...
          // Create notification
          if (rental) {
            const { data: lockerInfo } = await supabase.from('v_armario').select('cd_armario, nr_armario').eq('id_armario', rental.id_armario).maybeSingle();
